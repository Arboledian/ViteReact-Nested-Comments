...
//esto es un seeder para la database, copiado del repo
//dado que no se puede comentar en json, el archivo que se debe modificar para tomar el 
//seed es package.json

async function seed() {
  await prisma.post.deleteMany()
  await prisma.user.deleteMany()
  const kyle = await prisma.user.create({ data: { name: "Selen" } })
  const sally = await prisma.user.create({ data: { name: "Ewiwi" } })
